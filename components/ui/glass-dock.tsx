...
const MorphingIcon = ({ type, isActive, onClick, onMouseEnter }: { type: string, isActive: boolean, onClick: () => void, onMouseEnter?: () => void }) => {
    ...
    const animateStar = () => {
        ...
        gsap.fromTo(svg,
            ...
            { rotation: 0, scale: 1, duration: 0.8, ease: "elastic.out(1, 0.5)", transformOrigin: "50% 50%" }
        );
    };
    ...
    const handleMouseEnter = () => {
        ...
        else if (type === 'star') animateStar();
        else if (type === 'users') animateUsers();
    };

    const svgClass = cn(
        "w-[22px] h-[22px] fill-current transition-colors duration-200",
    ...
    if (type === 'star') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("star", isActive ? "active" : "")}>
                <svg viewBox="0 0 24 24" className={svgClass}>
                    <path ref={pathRef} d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
                </svg>
            </button>
        );
    }
    if (type === 'users') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("users", isActive ? "active" : "")}>
                <svg viewBox="0 0 24 24" className={svgClass}>
                    <path ref={pathRef} d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z" />
                </svg>
            </button>
        );
    }
            ...
            >
                <div
                    className={cn(
                        ...
                        "glass-border bg-black/80 dark:bg-black/80",
                ...
                >
                    ...
                    {items.map((el, index) => {
                        ...
                        // Check if title or animationType matches one of our animated icons
                        const type = (el.animationType || el.title).toLowerCase();
                        const isAnimated = ['home', 'blog', 'marker', 'email', 'linkedin', 'x', 'github', 'star', 'users'].includes(type);
